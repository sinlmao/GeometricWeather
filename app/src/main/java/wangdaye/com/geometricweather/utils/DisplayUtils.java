...
public class DisplayUtils {
    ...
    public static void setNavigationBarColor(Activity a, boolean isDay) {
        ...
        boolean color = sharedPreferences.getBoolean(
                a.getString(R.string.key_navigationBar_color),
                false);

        Window w = a.getWindow();
        if (color && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
    ...
    public static void setWindowTopColor(Activity a) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            boolean isDay = TimeUtils.getInstance(a).isDay;
